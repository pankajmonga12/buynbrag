...
$q = 'SELECT from_name, from_email, reply_to, title, plain_text, html_text, to_send, recipients FROM campaigns WHERE id = '.$campaign_id.' AND userID = '.get_app_info('main_userID');
$r = mysqli_query($mysqli, $q);
if ($r && mysqli_num_rows($r) > 0)
{
    while($row = mysqli_fetch_array($r))
...
if ($r && mysqli_num_rows($r) > 0)
{
    ...
    $q4 = 'SELECT id, query_str FROM queue WHERE campaign_id = '.$campaign_id.' AND sent = 0';
