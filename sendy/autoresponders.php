...
<?php 	
	...
	$q = 'SELECT ares_emails.id, ares_emails.from_name, ares_emails.from_email, ares_emails.reply_to, ares_emails.title, ares_emails.plain_text, ares_emails.html_text, ares_emails.time_condition, ares_emails.timezone, ares.list FROM ares, ares_emails WHERE ares_emails.ares_id = ares.id AND ares.type = 1';
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
	    {
			...
			{    
				if(substr($var, 0, 1)!="#" && substr($var, 0, 6)!="mailto")
				{
			...
			//select subscribers
			$q2 = 'SELECT * FROM subscribers WHERE list = '.$list.' AND unsubscribed = 0 AND bounced = 0 AND complaint = 0 AND confirmed = 1 GROUP BY email ORDER BY id ASC';
			$r2 = mysqli_query($mysqli, $q2);
			    ...
			    while($row = mysqli_fetch_array($r2))
			    {
					...
					if($join_date != '')
					{
						...
						if($time == $join_date_with_tc)
						{
							...
							$ql = 'SELECT id, link FROM links WHERE ares_emails_id = '.$ares_id;
					    	...
					    	$html_treated = str_replace('[webversion]', APP_PATH.'/w/'.short($subscriber_id).'/'.short($list).'/'.short($ares_id), $html_treated);
	...
	$q = 'SELECT ares_emails.id, ares_emails.from_name, ares_emails.from_email, ares_emails.reply_to, ares_emails.title, ares_emails.plain_text, ares_emails.html_text, ares_emails.time_condition, ares_emails.timezone, ares.list, custom_field FROM ares, ares_emails WHERE ares_emails.ares_id = ares.id AND ares.type = 2';
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
	...
	$q = 'SELECT ares_emails.id, ares_emails.from_name, ares_emails.from_email, ares_emails.reply_to, ares_emails.title, ares_emails.plain_text, ares_emails.html_text, ares_emails.time_condition, ares_emails.timezone, ares.list, custom_field FROM ares, ares_emails WHERE ares_emails.ares_id = ares.id AND ares.type = 3';
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
